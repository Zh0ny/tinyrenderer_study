...
TGAImage::TGAImage(const int w, const int h, const int bpp)
    : w(w), h(h), bpp((bpp == GRAYSCALE || bpp == RGB || bpp == RGBA)
                    ? static_cast<std::uint8_t>(bpp)
                    : throw std::invalid_argument("invalid bpp")),
      data(static_cast<size_t>(w) * static_cast<size_t>(h) *
              static_cast<size_t>(bpp),0) {}
...
bool TGAImage::load_rle_data(std::ifstream &in) {
  ...
  do {
    ...
    if (!(static_cast<unsigned>(in.get() - 1u < 65535u))) {
      return false;
    }
...
bool TGAImage::write_tga_file(const std::string filename, const bool vflip,
  ...
  if (!((static_cast<unsigned>(w) - 1u < 65535u) &&
        (static_cast<unsigned>(h) - 1u < 65535u))) {
    return false;
  }
